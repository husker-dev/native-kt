...
public abstract class AbstractAMD64CallingConvention extends CallingConvention {

    protected static final int RAX = 0, RCX = 1, RDX = 2, R8 = 8, R9 = 9, RDI = 7, RSI = 6;
    protected static final int XMM0 = 0, XMM1 = 1, XMM2 = 2, XMM3 = 3, XMM4 = 4, XMM5 = 5, XMM6 = 6, XMM7 = 7, XMM15 = 15;

    private final int[] hotspotIntReg;
    private final int[] hotspotFloatReg;

    private final int[] amd64IntReg;
    private final int[] amd64FloatReg;

    private final int shadowSpace;
    private final boolean continuousIndexing;

    protected AbstractAMD64CallingConvention(
        int[] hotspotIntReg,
        int[] hotspotFloatReg,
        int[] amd64IntReg,
        int[] amd64FloatReg,
        int shadowSpace,
        boolean continuousIndexing
    ) {
        this.hotspotIntReg = hotspotIntReg;
        this.hotspotFloatReg = hotspotFloatReg;
        this.amd64IntReg = amd64IntReg;
        this.amd64FloatReg = amd64FloatReg;
        this.shadowSpace = shadowSpace;
        this.continuousIndexing = continuousIndexing;
    }

    abstract void emitRegToReg(Buffer buf, Method method, RegToReg[] floatOps, RegToReg[] intOps);

    private int getAlignedStack(Method method){
        // (+8 is probably for calling shift)
        return shadowSpace + align16((method.getParameterCount()-amd64IntReg.length) * 8) + 8;
    }

    private boolean shouldUseStack(Method method){
        for(int i = 0, ints = 0, floats = 0; i < method.getParameterCount(); i++) {
            Class<?> type = method.getParameterTypes()[i];
            if (isIntegerType(type))
                ints++;
            if(isFloatingPointType(type)) 
                floats++;
            if(ints > amd64IntReg.length || floats > amd64FloatReg.length)
                return true;
        }
        return false;
    }
    ...
    protected void emitConversion(Buffer buf, Method method) {
        ...
        for (int i = 0, floats = 0, ints = 0, jStack = 0, cStack = 0; i < method.getParameterCount(); i++) {
            ...
            if(isFloatingPointType(type)){
                int r = continuousIndexing ? floats : i;

                if(r < amd64FloatReg.length)
                    regToReg.add(new RegToReg(hotspotFloatReg[floats], amd64FloatReg[r], type));
                else if(floats < hotspotFloatReg.length)
                    regToStack.add(new RegToStack(hotspotFloatReg[floats], shadowSpace + 8 * cStack++, type));
                else
                    stackToStack.add(new StackToStack(stackShift + 8 * jStack++, shadowSpace + 8 * cStack++, type));
                floats++;
            } else {
                int r = continuousIndexing ? ints : i;

                if(r < amd64IntReg.length)
                    regToReg.add(new RegToReg(hotspotIntReg[ints], amd64IntReg[r], type));
                else if(ints < hotspotIntReg.length)
                    regToStack.add(new RegToStack(hotspotIntReg[ints], shadowSpace + 8 * cStack++, type));
                else
                    stackToStack.add(new StackToStack(stackShift + 8 * jStack++, shadowSpace + 8 * cStack++, type));
                ints++;
            }
        ...
        emitRegToReg(
                buf, method,
                regToReg.stream().filter(e -> isFloatingPointType(e.type)).toArray(RegToReg[]::new),
                regToReg.stream().filter(e -> isIntegerType(e.type)).toArray(RegToReg[]::new)
        );
    }
