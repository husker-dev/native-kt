...
internal fun configureJs(
    ...
    File(cmakeDir, "CMakeLists.txt").writeText($$"""
        ...
        set_target_properties(lib$${module.name} PROPERTIES LINK_FLAGS "-s -lembind -s --no-entry -s ALLOW_MEMORY_GROWTH=1 -s ALLOW_TABLE_GROWTH=1 -s MODULARIZE=1 -s EXPORT_ES6=1 -s WASM_BIGINT=$${if(extension.useJsBigInt) "1" else "0"} -s EXPORTED_RUNTIME_METHODS=UTF8ToString,stringToUTF8,lengthBytesUTF8,HEAP32,addFunction -s EXPORTED_FUNCTIONS=$$exportedStr")
    """.trimIndent())
